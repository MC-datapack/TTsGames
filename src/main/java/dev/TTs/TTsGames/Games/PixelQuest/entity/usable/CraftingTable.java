...
@Buggy(reason = {"Klick area to open is broken", "Items Transfer by clicking on a recipe output is broken", "Available recipes are broken"})
public class CraftingTable extends Entity implements InventoryOwner {
    ...
    public void moveItemsToCraftingTable(RecipeFormats.Crafting recipe) {
        ...
        for (Map.Entry<Integer, Item> entry : inputMap.entrySet()) {
            ...
            for (int i = 0; i < WorldSaving.world.player.inventory.getMaxSize(); i++) {
                ItemStack itemStack = WorldSaving.world.player.inventory.getItemStack(i);
                if (itemStack != null && itemStack.getItem().equals(item)) {
                    try {
                        int count = itemStack.getCount();
                        inventory.replaceSlot(slot, new ItemStack(item, count));
                        WorldSaving.world.player.inventory.removeItem(new ItemStack(item, count));
        ...
        try {
            WorldSaving.world.player.inventory.addItem(outputItem);
        } catch (InventoryOutOfBoundsException e) {
    ...
    public Identifier id() {
        return Identifier.ofPixelQuest("crafting_table");
    }
    ...
    private void readObject(ObjectInputStream ois) throws IOException, ClassNotFoundException {
        logger.debug("Try loading Crafting Table");
        try {
            ois.defaultReadObject();
        } catch (IOException e) {
            logger.error("Failed loading Crafting Table");
        }
        recipes = PixelQuestJsonReader.getAllCraftingTableRecipes();
    }
}
